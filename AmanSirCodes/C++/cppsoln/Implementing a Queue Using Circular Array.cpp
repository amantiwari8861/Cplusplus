...
{
if((FRONT == 0 && REAR == max-1) || (FRONT == REAR + 1)) /*This
statement checks for the overflow condition. */
{
cout<<"\nQueue overflow\n"<<endl;
return;
}
...
REAR = REAR + 1; /* If REAR is not at the last position, then its
...
"<<queue_array[FRONT]<<endl<<endl;
if (FRONT == REAR) /* Checks if the queue has one
element. */
{
FRONT = -1;
REAR = -1;
...
then the value of FRONT is set to 0 i.e to the first element of the
array. */
if (FRONT == max - 1)
FRONT = 0;
else
...
{
while (FRONT_position <= max - 1) /* Traverses the queue till the
last position of the array. */
{
cout<<queue_array[FRONT_position] << " ";
FRONT_position++;
}
FRONT_position = 0; /* Set the FRONT position to
the first element of the
array. */
while (FRONT_position <= REAR_position)
/* Traverses the array till the last element
